...
if (!videoProjectId) {
  console.error(
    "Missing videoProjectId. Usage: node scripts/render-final-video-by-id.mjs <videoProjectId> <generatedVideoId>",
  );
  process.exit(1);
}
...
try {
  ...
  const rawOutputLocation = path.resolve(rendersDir, `final-${videoProjectId}-raw.mp4`);
  const outputLocation = path.resolve(rendersDir, `final-${videoProjectId}.mp4`);

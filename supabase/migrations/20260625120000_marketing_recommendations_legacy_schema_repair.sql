-- Rebuild marketing_recommendations when an older manual/legacy schema is present.
-- The app expects recommendation_key, category, message, metrics, patterns, etc.
-- Legacy tables used recommendation_type, insight, action, source instead.
...
begin
  if exists (
    ...
    where table_schema = 'public'
      and table_name = 'marketing_recommendations'
    ...
    drop table public.marketing_recommendations;
  end if;
end $$;
...
drop policy if exists "Users can manage own marketing recommendations"
  on public.marketing_recommendations;

drop policy if exists "marketing_recommendations: select own brand"
  on public.marketing_recommendations;
...
drop policy if exists "marketing_recommendations: insert own brand"
  on public.marketing_recommendations;
...
drop policy if exists "marketing_recommendations: delete own brand"
  on public.marketing_recommendations;
